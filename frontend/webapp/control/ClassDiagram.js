sap.ui.define(
  ["sap/ui/core/Control", "com/extension-inspector/extension-inspector/thirdparty/mermaid"],
  (Control, mermaidjs) => {
    "use strict"
    ...
    return Control.extend("com.extension-inspector.extension-inspector.control.ClassDiagram", {
      ...
      _generateDiagramSyntax(oDeclarations) {
        ...
        if (aUniqueObjects.length === 0) {
          sDiagramSyntax += this._getDefinitionSyntax(
            {
              SourceObjectType: this.getType(),
              SourceObjectName: this.getObject(),
            },
            aDefinitions,
          )
        }
      ...
      _getDefinitionSyntax(oUniqueObject, aDefinitions) {
        const aRelevantDefinitions = aDefinitions.filter(
          (oDefinition) =>
      ...
      _getObjectSyntax(aDeclarations, sObject) {
        ...
        const aFilteredDeclarations = aDeclarations.filter(
          (oDeclaration) =>
            oDeclaration.ObjectName === sObject && aRelevantComponents.includes(oDeclaration.DefinitionType),
        )
        ...
        for (const oDeclaration of aFilteredDeclarations) {
          ...
          if (oDeclaration.DefinitionType === "METHODS" || oDeclaration.DefinitionType === "CLASS-METHODS")
            sLine += `()`
          if (
            (oDeclaration.DefinitionType === "DATA" || oDeclaration.DefinitionType === "CLASS-DATA") &&
            oDeclaration.DefinitionName
          )
            sLine += `: ${oDeclaration.Name2}`
      ...
      _getConnectionSyntax(aDeclarations) {
        ...
        const aFilteredDeclarations = aDeclarations.filter((oDeclaration) =>
