    ...
    return Control.extend("com.extension-inspector.extension-inspector.control.RAPDiagram", {
        ...
        renderer(oRm, oControl) {
            ...
            if (oControl.sDiagramSyntax) {
                oRm.write(`<style>svg { cursor: grab; }</style><div id="diagram-container" style="width:100%; height:50vh; overflow:hidden;"><pre class="mermaid" style="height: 100% !important;">${oControl.sDiagramSyntax}</pre></div>`);
            } else {
        ...
        _generateDiagramSyntax(oDeclarations) {
            ...
            aRelations = aRelations.filter(oRelation => this._isConnectedOnlyZUndirected(
                aRelations,
                oRelation.SourceObjectType, oRelation.SourceObjectName, // Start
                sObjectType, sObjectName // Target
            ))
            ...
            aUniqueObjects.forEach(oUniqueObject => {
                const sDefinitionName = aDefinitions.find(oDefinition => oDefinition.SourceObjectType === oUniqueObject.SourceObjectType && oDefinition.SourceObjectName === oUniqueObject.SourceObjectName)?.DefinitionName
                ...
                let sLink = window.location.hash.split("&")[0] + "&/objects/" + oUniqueObject.SourceObjectType + "/" + oUniqueObject.SourceObjectName
